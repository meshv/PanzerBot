...
ps.lookup({command: 'mongod'}, function lookupMongo(err, result){
...
WebProcess.on("message", function handleWebsiteIPC(msg){
    ...
    switch(msg.event){
        case "startBot":
            ...
            break;
        case "endBot":
            BotProcess.send({"event":"closeSelf"}); // Let the bot handle it's own closing to ensure that all ties are wrapped up
            break;
    }
});
